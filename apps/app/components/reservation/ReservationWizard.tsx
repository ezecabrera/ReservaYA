...
export function ReservationWizard({ venue, prefill, sectors: initialSectors }: ReservationWizardProps) {
  // Si llegamos con prefill completo (date + time + partySize), salteamos el
  // primer paso de datetime y arrancamos directo en sector. Esto pasa cuando
  // el user eligió día/hora/personas en el panel sticky del venue detail (desktop).
  const hasCompletePrefill = !!(prefill?.date && prefill?.time && prefill?.partySize)
  const [step, setStep] = useState<WizardStep>(hasCompletePrefill ? 'sector' : 'datetime')
  ...
  // Auto-saltar el paso de sector cuando:
  //   - llegamos con prefill completo (date+time+party)
  //   - el venue tiene un solo sector (o ninguno), entonces no hay elección que hacer
  // En ese caso disparamos handleSectorPicked(null) al montar, que lockea mesa y va a menu.
  const autoSkipTriggered = useRef(false)
  useEffect(() => {
    if (autoSkipTriggered.current) return
    if (hasCompletePrefill && !hasMultipleSectors && step === 'sector') {
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => {
              // Si arrancamos con prefill completo (desde venue detail desktop),
              // el "volver" lleva al venue detail — no al step datetime que nunca
              // mostramos. Si no, back normal al step anterior.
