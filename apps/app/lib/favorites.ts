...
function writeFavorites(list: string[]): void {
  ...
  try {
    ...
    window.dispatchEvent(new CustomEvent('untoque-favorites-changed'))
  } catch {
    /* silent */
  }
}
...
export function useFavorites(): {
  ...
  useEffect(() => {
    ...
    window.addEventListener('untoque-favorites-changed', handler)
    return () => {
      window.removeEventListener('storage', handler)
      window.removeEventListener('untoque-favorites-changed', handler)
    }
  }, [])
