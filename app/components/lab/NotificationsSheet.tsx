...
function readLocalReadIds(): Set<string> {
  ...
  try {
    const raw = localStorage.getItem('reservaya-notifs-read')
    return new Set(raw ? JSON.parse(raw) : [])
  } catch { return new Set() }
}

function saveLocalReadIds(ids: Set<string>) {
  try { localStorage.setItem('reservaya-notifs-read', JSON.stringify([...ids])) } catch { /* */ }
}
...
export function NotificationsSheet({ open, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col justify-end">
      ...
      />
      <div className="relative bg-bg rounded-t-3xl max-h-[85vh] flex flex-col shadow-[0_-8px_32px_rgba(0,0,0,0.12)]">
